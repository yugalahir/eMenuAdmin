...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_layout($title)
	{
	...
	$this->set_css($this->default_css_path.'/ui/simple/'.grocery_CRUD::JQUERY_UI_CSS);
	$this->set_css($this->default_css_path.'/jquery_plugins/jquery.ui.datetime.css');
	$this->set_css($this->default_css_path.'/jquery_plugins/timepicker.css');
	$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/ui/'.grocery_CRUD::JQUERY_UI_JS);
	$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/bootstrap-timepicker.js');
	...
	protected function get_set_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select' data-placeholder='$select_title' style='width:299px;' >";
...
#222
	protected function get_upload_file_readonly_input($field_info,$value)
	{
		...
		//$value = !empty($value) ?  '555 <a  href="'.$file.'" target="_blank">'.$value.'</a>' : '';
	...
	protected function get_relation_n_n_input($field_info_type, $selected_values)
	{
		...
		else
		{
			...
			$width_style = $has_priority_field || $is_ie_7 ? '' : 'width:299px;';
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $default_theme		= 'datatables';//// changed from flexigrid to datatables
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	...
	public function render()
	{
		...
		return $this->get_layout($this->page_title);
	}
