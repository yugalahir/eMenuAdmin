...
(/** @lends <global> */function($, window, document, undefined) {
	...
	var DataTable = function( oInit )
	{
		...
		/**
		 * Figure out how to reorder a table list
		 *  @param {object} oSettings dataTables settings object
			...
			for ( iColumn=0, iColumns=oSettings.aoColumns.length ; iColumn<iColumns ; iColumn++ )
			{
				...
				if ( bAutoType || bRender || bClass || !bVisible )
				{
					for ( iRow=0, iRows=oSettings.aoData.length ; iRow<iRows ; iRow++ )
					{
						...
						{
							nCell.innerHTML = _fnGetCellData( oSettings, iRow, iColumn, 'table' );
						}
						...
						if ( oCol.fnCreatedCell )
						{
							oCol.fnCreatedCell.call( oSettings.oInstance,
								nCell, _fnGetCellData( oSettings, iRow, iColumn, 'table' ), oData._aData, iRow, iColumn
							);
						}
					}
				}
		 ...
		 *  @param {string} sSpecific data get type ('table', 'type' 'filter' 'sort')
		 *  @returns {*} Cell data
		 *  @memberof DataTable#oApi
		 */
		...
		function _fnRender( oSettings, iRow, iCol )
		{
			...
			}, _fnGetCellData(oSettings, iRow, iCol, 'table') );
		}
			...
			if ( oData.nTr === null )
			{
				...
				for ( var i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					nTd.innerHTML = (typeof oCol.fnRender === 'function' && (!oCol.bUseRendered || oCol.mDataProp === null)) ?
						...
						_fnGetCellData( oSettings, iRow, i, 'table' );
					...
					{
						oCol.fnCreatedCell.call( oSettings.oInstance,
							nTd, _fnGetCellData( oSettings, iRow, i, 'table' ), oData._aData, iRow, i
						);
					}
				}
		...
		/**
		 * Insert the required TR nodes into the table for table
		 *  @param {object} oSettings dataTables settings object
		 *  @memberof DataTable#oApi
		 */
		function _fnDraw( oSettings )
		{
			...
			/* 
			 * Need to remove any old row from the table - note we can't just empty the tbody using
		 ...
		 *  @param {array} json.aaData The data to table on this page
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			jqFilter.bind( 'keyup.DT', function(e) {
				/* Update all other filter input elements for the new table */
		...
		function _fnFilter( oSettings, sInput, iForce, bRegex, bSmart, bCaseInsensitive )
		{
			...
			else
			{
				...
				{
					/* Nuke the old table array - we are going to rebuild it */
					oSettings.aiDisplay.splice( 0, oSettings.aiDisplay.length);
		...
		/**
		 * Generate the node required for the info table
		 *  @param {object} oSettings dataTables settings object
		...
		/**
		 * Update the information elements in the table
		 *  @param {object} oSettings dataTables settings object
		 *  @memberof DataTable#oApi
		 */
		...
		function _fnInitialise ( oSettings )
		{
			...
			if ( oSettings.sAjaxSource !== null && !oSettings.oFeatures.bServerSide )
			{
				...
				oSettings.fnServerData.call( oSettings.oInstance, oSettings.sAjaxSource, aoData, function(json) {
					...
					/* Reset the init table for cookie saving. We've already done a filter, and
		...
		/**
		 * Generate the node required for user table length changing
		 *  @param {object} oSettings dataTables settings object
		 *  @returns {node} table length feature node
		 *  @memberof DataTable#oApi
		 */
		function _fnFeatureHtmlLength ( oSettings )
		{
			...
			/*
			 * Set the length to the current table length - thanks to Andrea Pavlovic for this fix,
			...
			$('select', nLength).bind( 'change.DT', function(e) {
				...
				/* Update all other length options for the new table */
		...
		function _fnCalculateEnd( oSettings )
		{
			...
			{
				/* Set the end point of the table - based on how many elements there are
				 * still to table
				 */
		...
		function _fnFeatureHtmlPaginate ( oSettings )
		{
			...
			/* Add a draw callback for the pagination on first instance, to update the paging table */
		...
		/**
		 * Alter the table settings to change the page
		 *  @param {object} oSettings dataTables settings object
		...
		function _fnPageChange ( oSettings, mAction )
		{
			...
			else if ( mAction == "next" )
			{
				...
				{
					/* Make sure we are not over running the table array */
		...
		/**
		 * table or hide the processing indicator
		 *  @param {object} oSettings dataTables settings object
		...
		function _fnScrollDraw ( o )
		{
			...
			if ( o.oScroll.sY === "" )
			{
				 ...
				 * the scrollbar height from the visible table, rather than adding it on. We need to
		...
		/**
		 * Apply a given function to the table child nodes of an element array (typically
		 * TD children of TR rows
		 *  @param {function} fn Method to apply to the objects
		 *  @param array {nodes} an1 List of elements to look through for table children
		...
		function _fnGetMaxLenString( oSettings, iCol )
		{
			...
			{
				var s = _fnGetCellData( oSettings, i, iCol, 'table' )+"";
				s = s.replace( /<.*?>/g, "" );
				if ( s.length > iMax )
				{
					iMax = s.length;
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
			...
			{
				oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
				oSettings._iDisplayStart = 0; /* reset table back to page 0 */
				_fnCalculateEnd( oSettings );
				_fnDraw( oSettings );
			}
		}
		...
		function _fnSortAttachListener ( oSettings, nNode, iDataIndex, fnCallback )
		{
			_fnBindAction( nNode, {}, function (e) {
				 ...
				 * appear to allow the rendering engine to update the table while it is still excuting
				 * it's thread (well - it does but only after long intervals). This means that the 
				 * 'processing' table doesn't appear for a table sort. To break the js thread up a bit
		 ...
		 *  @param {object} oSettings dataTables settings object
		 *  @param {int} iLevel log error messages, or table them to the user
		...
		this.fnClose = function( nTr )
		{
			...
			for ( var i=0 ; i<oSettings.aoOpenRows.length ; i++ )
			{
				if ( oSettings.aoOpenRows[i].nParent == nTr )
				{
					...
					if ( nTrParent )
					{
						/* Remove it if it is currently on table */
		 ...
		 * on table on the page, with the HTML contents that is passed into the 
		 ...
		 *  @param {int} iCol The column whose table should be changed
