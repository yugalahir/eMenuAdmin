$(function(){
					...
					$('#crudForm').ajaxSubmit({
						...
						success: function(result){
							...
							$("#FormLoading").fadeOut("slow");
                                                        //temp fix for removing javascript from ajax grocery crud
                                                        var pos = result.indexOf("<script");
                                                        var tmpstr=result;
                                                        if(pos>0)
                                                        {
                                                            tmpstr = result.substr(0, pos);
                                                        }
                                                        //!--end temp fix
                                                        data = $.parseJSON( tmpstr );
	...
	if( $('#cancel-button').closest('.ui-dialog').length === 0 ) {
        ...
        if( $('#back-button').closest('.ui-dialog').length === 0 ) {
